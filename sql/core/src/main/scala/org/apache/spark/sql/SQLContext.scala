...
class SQLContext private[sql](
  ...
  protected[sql] def applySchemaToPythonRDD(
    ...
    val rowRdd = rdd.map(r => {
      ...
      val   file = new java.io.File("/home/gt/scala_worker34.txt")
      ...
      val temp = EvaluatePython.fromJava(r, schema).asInstanceOf[InternalRow]
   ...
   * Apply a schema defined by the schemaString to an RDD. It is only used by PySpark.
   */
  protected[sql] def applySchemaToPhpRDD(
                                             rdd: RDD[Array[Any]],
                                             schemaString: String): DataFrame = {
    val schema = parseDataType(schemaString).asInstanceOf[StructType]
    applySchemaToPhpRDD(rdd, schema)
  }
  ...
  protected[sql] def applySchemaToPhpRDD(
    ...
    val rowRdd = rdd.map(r => {
      ...
      val   file = new java.io.File("/home/gt/scala_worker33.txt")
      val   fos = new java.io.FileWriter(file,true);
      val   osw = new BufferedWriter(fos);
      osw.write("#####" + r)
      osw.write("#####" + schema)
      ...
      for(e<-r.asInstanceOf[Array[_]]){
        osw.write("$$$$$" + e)
        osw.newLine()
        osw.flush()
      }

      val temp = EvaluatePhp.fromJava(r, schema).asInstanceOf[InternalRow]
