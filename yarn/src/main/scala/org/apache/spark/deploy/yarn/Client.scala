...
private[spark] class Client(
  ...
  private def setupLaunchEnv(
    ...
    val (pyFiles, pyArchives) = args.pyFiles.partition(_.endsWith(".py"))//pyFiles应该是客户的程序
    if (pyFiles.nonEmpty) {
    ...
    val phpPath = new ListBuffer[String]()
    val (phpFiles, phpArchives) = args.phpFiles.partition(_.endsWith(".php"))
    if (phpFiles.nonEmpty) {
      phpPath += buildPath(YarnSparkHadoopUtil.expandEnvironment(Environment.PWD),
        LOCALIZED_PHP_DIR)
    }
    //pySparkArchives是传进来的
    (pySparkArchives ++ phpArchives).foreach { path =>
      val uri = Utils.resolveURI(path)
      if (uri.getScheme != LOCAL_SCHEME) {
        phpPath += buildPath(YarnSparkHadoopUtil.expandEnvironment(Environment.PWD),
          new Path(uri).getName())
    ...
    // Finally, update the Spark config to propagate PHPPATH to the AM and executors.
    if (phpPath.nonEmpty) {
      val phpPathStr = (sys.env.get("PHPPATH") ++ phpPath)
        .mkString(YarnSparkHadoopUtil.getClassPathSeparator)
      env("PHPPATH") = phpPathStr
      sparkConf.setExecutorEnv("PHPPATH", phpPathStr)
    }
  ...
  private def createContainerLaunchContext(newAppResponse: GetNewApplicationResponse)
    ...
    val pySparkArchives =
      if (sparkConf.getBoolean("spark.yarn.isPython", false)) {
        findPySparkArchives()
      } else if(sparkConf.getBoolean("spark.yarn.isPhp", false)){
        findSparkPhpArchives()
      } else {
        Nil
      }
  ...
  private def findSparkPhpArchives(): Seq[String] = {
    sys.env.get("SPARKPHP_ARCHIVES_PATH")
      .map(_.split(",").toSeq)
      .getOrElse {
      val phpLibPath = Seq(sys.env("SPARK_HOME"), "php", "yarn").mkString(File.separator)
      val phpArchivesFile = new File(phpLibPath, "SparkPHP.zip")
      require(phpArchivesFile.exists(),
        "SparkPHP.zip not found; cannot run sparkPhp application in YARN mode.")
      Seq(phpArchivesFile.getAbsolutePath())
    }
  }
...
object Client extends Logging {
  ...
  val LOCALIZED_PHP_DIR = "__phpfiles__"
