...
object SparkSubmit {
  ...
  private val PHPSPARK_SHELL = "phpspark-shell"
  ...
  private[deploy] def prepareSubmitEnvironment(args: SparkSubmitArguments)
    ...
    if (!StringUtils.isBlank(resolvedMavenCoordinates)) {
      ...
      if (args.isPHP){
        args.phpFiles = mergeFileLists(args.phpFiles, resolvedMavenCoordinates)
      }
    }
    ...
    // If we're running a python app, set the main class to our specific python runner
    if (args.isPHP && deployMode == CLIENT) {
      if (args.primaryResource == PHPSPARK_SHELL) {
        args.mainClass = "org.apache.spark.api.python.PhpGatewayServer"
      } else {
        // If a python file is provided, add it to the child arguments and list of files to deploy.
        // Usage: PythonAppRunner <main python file> <extra python files> [app arguments]
        args.mainClass = "org.apache.spark.deploy.PhpRunner"
        args.childArgs = ArrayBuffer(args.primaryResource, args.phpFiles) ++ args.childArgs
        if (clusterManager != YARN) {
          // The YARN backend distributes the primary file differently, so don't merge it.
          args.files = mergeFileLists(args.files, args.primaryResource)
        }
  ...
  private[deploy] def isUserJar(res: String): Boolean = {
    !isShell(res) && !isPython(res) && !isInternal(res) && !isR(res)  && !isPHP(res)
  }
  ...
  private[deploy] def isPHP(res: String): Boolean = {
    res != null && res.endsWith(".php") || res == PHPSPARK_SHELL
  }
